...
export default function FormularioAmbiental({ cultivoId, onGuardado, onCancelar }) {
  ...
  // F-M02 FIX: validar rangos agronómicos
  const validar = () => {
    if (form.temperatura !== "" && (parseFloat(form.temperatura) < -10 || parseFloat(form.temperatura) > 55))
      return "Temperatura debe estar entre -10 y 55 °C";
    if (form.humedad_relativa !== "" && (parseFloat(form.humedad_relativa) < 0 || parseFloat(form.humedad_relativa) > 100))
      return "Humedad debe estar entre 0 y 100 %";
    if (form.precipitacion_mm !== "" && parseFloat(form.precipitacion_mm) < 0)
      return "La precipitación no puede ser negativa";
    return null;
  };
  ...
  return (
    <form onSubmit={enviar} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.85rem" }}>
        <Campo label="Temperatura del aire (°C)"        name="temperatura"      value={form.temperatura}
          onChange={cambio} placeholder="Ej: 22.5" ayuda="Rango: -10 a 55" />
        <Campo label="Humedad del ambiente (%)"   name="humedad_relativa" value={form.humedad_relativa}
          onChange={cambio} placeholder="Ej: 78.3" ayuda="Rango: 0 a 100" />
        <Campo label="Lluvia registrada (mm)"     name="precipitacion_mm" value={form.precipitacion_mm}
          onChange={cambio} placeholder="Ej: 12.0" />
        <Campo label="Radiación solar (W/m²)" name="radiacion_solar"  value={form.radiacion_solar}
          onChange={cambio} placeholder="Ej: 420.0" />
        <Campo label="Velocidad del viento (km/h)"          name="velocidad_viento" value={form.velocidad_viento}
