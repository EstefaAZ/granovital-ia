# ==============================================================
# modulo_05_trazabilidad / app/core/security.py
# JWT y RBAC — RN-01
# ==============================================================
...
def get_current_user_rol(payload: dict = Depends(get_current_user_payload)) -> str:
    ...
    def _check(payload: dict = Depends(get_current_user_payload)) -> str:
        ...
        if rol not in roles:
            raise HTTPException(
                ...
                detail=f"Acceso denegado. Rol '{rol}' no autorizado para esta operacion.",
            )
        return rol
    return _check
